module.exports = function(grunt) {
  grunt.initConfig({
    ...
    uglify: {
      options: {
        ...
        banner: '/*! <%= pkg.title || pkg.name %> - v<%= pkg.version %> - ' + '(c) <%= grunt.template.today("yyyy") %> <%= pkg.author.name %>;' + ' Licensed <%= _.pluck(pkg.licenses, "type").join(", ") %> */\n'
      },
      build: {
        src: '<%= dirs.jsDir %>/<%= pkg.name %>.js',
    ...
    sizediff: {
      all: {
        src: ['<%= dirs.jsDir %>/<%= pkg.name %>.js', '<%= dirs.jsDir %>/<%= pkg.name %>.min.js']
      }
    },
    coffee: {
      nano: {
  ...
  return grunt.registerTask('server', ['default', 'connect:server', 'watch']);
};
